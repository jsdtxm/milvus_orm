...
import asyncio
from milvus_orm import (
    connect, disconnect, Model, 
    INT64, VARCHAR, FLOAT, FLOAT_VECTOR, JSON
)
...
class Product(Model):
    ...
    id = INT64(primary_key=True, auto_id=False)
...
class User(Model):
    ...
    user_id = INT64(primary_key=True)
    username = VARCHAR(max_length=100)
    email = VARCHAR(max_length=255)
    preferences = JSON(nullable=True)
...
async def main():
    ...
    try:
        ...
        if await User.create_collection():
            print("✓ User集合创建成功")
        else:
            print("⚠ User集合已存在")
        ...
        # 获取单个对象
        try:
            found_product = await Product.objects().get(id=1)
        ...
        similar_products = await Product.objects().search(
            vector=query_vector,
            field_name="vector",
        ...
        # 6. 更新数据
        print("\n6. 更新数据...")
        product_to_update = await Product.objects().get(id=1)
        await product_to_update.update(
            price=899.99,
            name="智能手表Pro",
        ...
        # 验证更新
        updated_product = await Product.objects().get(id=1)
